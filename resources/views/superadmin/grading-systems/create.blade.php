        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Create New Grading System</h3>
                </div>
                <form action="{{ route('superadmin.grading-systems.store') }}" method="POST">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="program_category" id="program_category" class="form-control @error('program_category') is-invalid @enderror" required>
                                <option value="all" {{ old('program_category') == 'all' ? 'selected' : '' }}>All Programs (Default)</option>
                                <option value="health" {{ old('program_category') == 'health' ? 'selected' : '' }}>Health Programs (CMT, PST, EHT)</option>
                                <option value="non_health" {{ old('program_category') == 'non_health' ? 'selected' : '' }}>Non-Health Programs (Business, etc.)</option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="min_score" id="min_score" class="form-control @error('min_score') is-invalid @enderror" value="{{ old('min_score') }}" required>
                            @error('min_score')<span class="invalid-feedback">{{ $message }}</span>@enderror
                        </div>

                        <div class="form-group">
                            <label for="max_score">Maximum Score *</label>
                            <input type="number" name="max_score" id="max_score" class="form-control @error('max_score') is-invalid @enderror" value="{{ old('max_score') }}" required>
                            @error('max_score')<span class="invalid-feedback">{{ $message }}</span>@enderror
                        </div>

                        <div class="form-group">
                            <label for="grade">Grade *</label>
                            <input type="text" name="grade" id="grade" class="form-control @error('grade') is-invalid @enderror" value="{{ old('grade') }}" maxlength="2" required>
                            @error('grade')<span class="invalid-feedback">{{ $message }}</span>@enderror
                        </div>

                        <div class="form-group">
                            <label for="grade_point">Grade Point *</label>
                            <input type="number" step="0.01" name="grade_point" id="grade_point" class="form-control @error('grade_point') is-invalid @enderror" value="{{ old('grade_point') }}" required>
