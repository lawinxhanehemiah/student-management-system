...
<div class="nxl-content">
    <div class="page-header">
        <div class="page-header-title">
            <h5>Edit Module: {{ $module->code }} - {{ $module->name }}</h5>
        </div>
                ...
                <div class="row">
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="nta_level" id="nta_level" class="form-control @error('nta_level') is-invalid @enderror" required>
                            ...
                            @for($i=1; $i<=6; $i++)
                                <option value="{{ $i }}" {{ old('nta_level', $module->nta_level) == $i ? 'selected' : '' }}>Level {{ $i }}</option>
                            @endfor
                        </select>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="type" id="type" class="form-control @error('type') is-invalid @enderror" required>
                            ...
                            <option value="Core" {{ old('type', $module->type) == 'Core' ? 'selected' : '' }}>Core (Lazima)</option>
                            <option value="Fundamental" {{ old('type', $module->type) == 'Fundamental' ? 'selected' : '' }}>Fundamental (Msingi)</option>
                            <option value="Elective" {{ old('type', $module->type) == 'Elective' ? 'selected' : '' }}>Elective (Chaguzi)</option>
                        </select>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.5" name="default_credits" id="default_credits" 
                               ...
                               value="{{ old('default_credits', $module->default_credits) }}" required>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="number" step="0.5" name="pass_mark" id="pass_mark" 
                               ...
                               value="{{ old('pass_mark', $module->pass_mark) }}" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="department_id" id="department_id" class="form-control @error('department_id') is-invalid @enderror">
                            ...
                            @foreach($departments as $id => $name)
                                <option value="{{ $id }}" {{ old('department_id', $module->department_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                        @error('department_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        <small class="form-text text-muted">Optional: Assign module to a department.</small>
                    </div>
                    <div class="col-md-6 mb-3">
