<?php
// app/Http/Controllers/SuperAdmin/SupplementaryFeeController.php
...
{
    /**
     * Display supplementary fees for a programme
     */
    public function index($programmeId, Request $request)
    {
        try {
            ...
            $query = SupplementaryFee::where('programme_id', $programme->id)
                ->with('academicYear');
            ...
            return view('superadmin.supplementary-fees.index', 
    ...
    /**
     * Show form to create supplementary fee
     */
    public function create($programmeId)
    {
        try {
            ...
            if ($academicYears->isEmpty()) {
                return redirect()->route('superadmin.programmes.supplementary-fees.index', $programme->id)
                    ->with('error', 'No active academic years found.');
            }
            
            return view('superadmin.supplementary-fees.create', 
    ...
    /**
     * Store supplementary fee - ONLY TOTAL FEE
     */
    public function store(Request $request, $programmeId)
    {
        try {
            ...
            $validated = $request->validate([
                ...
                // NO is_active validation
            ]);
            
            // Check if exists
            $exists = SupplementaryFee::where('programme_id', $programme->id)
            ...
            if ($exists) {
                return back()
                    ->withErrors(['level' => 'Supplementary fee already exists for this programme, academic year, level and semester combination.'])
                    ->withInput();
            }
            ...
            SupplementaryFee::create($validated);
            
            return redirect()->route('superadmin.programmes.supplementary-fees.index', $programme->id)
                ->with('success', 'Supplementary fee added successfully for ' . $programme->name);
    ...
    /**
     * Edit supplementary fee
     */
    public function edit($programmeId, $feeId)
    {
        try {
            $programme = Programme::findOrFail($programmeId);
            $fee = SupplementaryFee::where('programme_id', $programme->id)
                ->findOrFail($feeId);
            ...
            return view('superadmin.supplementary-fees.edit', 
                compact('programme', 'fee', 'academicYears', 'levels', 'semesters'));
                
        } catch (\Exception $e) {
            return redirect()->route('superadmin.programmes.supplementary-fees.index', $programmeId)
                ->with('error', 'Fee not found: ' . $e->getMessage());
        }
    }
    
    /**
     * Update supplementary fee - ONLY TOTAL FEE
     */
    public function update(Request $request, $programmeId, $feeId)
    {
        try {
            ...
            return redirect()->route('superadmin.programmes.supplementary-fees.index', $programme->id)
                ->with('success', 'Supplementary fee updated successfully for ' . $programme->name);
    ...
    public function destroy($programmeId, $feeId)
    {
        try {
            ...
            return redirect()->route('superadmin.programmes.supplementary-fees.index', $programme->id)
                ->with('success', 'Supplementary fee deleted successfully.');
    ...
    public function copyFees(Request $request, $programmeId)
    {
        try {
            ...
            $fromFees = SupplementaryFee::where('programme_id', $programme->id)
            ...
            return redirect()->route('superadmin.programmes.supplementary-fees.index', $programme->id)
                ->with('success', 'Copied ' . $copiedCount . ' supplementary fees.');
    ...
    public function bulkCreate($programmeId)
    {
        try {
            ...
            return view('superadmin.supplementary-fees.bulk-create', 
                compact('programme', 'academicYears', 'levels', 'semesters'));
        } catch (\Exception $e) {
            return redirect()->route('superadmin.programmes.supplementary-fees.index', $programmeId)
                ->with('error', 'Error: ' . $e->getMessage());
    ...
    public function bulkStore(Request $request, $programmeId)
    {
        try {
            ...
            return redirect()->route('superadmin.programmes.supplementary-fees.index', $programme->id)
                ->with('success', "Created {$createdCount} supplementary fees for {$programme->name}. Skipped {$skippedCount} (already exist).");
    ...
    {
        try {
            $fee = SupplementaryFee::where('programme_id', $programmeId)
                ->where('level', $level)
                ->where('semester', $semester)
                ->where('is_active', 1)
                ->first();
    ...
    {
        try {
            $exists = SupplementaryFee::where('programme_id', $request->programme_id)
